...
class mnist_data_prep:
    ...
    def __init__(self):
        images = read_images(MNIST_ROOT / 'train-images-idx3-ubyte.gz')
        labels = read_labels(MNIST_ROOT / 'train-labels-idx1-ubyte.gz')
        images = images * (1. / 255.)
        images = images.reshape((images.shape[0], 28, 28, 1))
        self.X_train, self.X_val, self.labels_train, self.labels_val = train_test_split(images, labels, train_size=0.8,
                                                                                        test_size=0.2)

        images = read_images(MNIST_ROOT / 't10k-images-idx3-ubyte.gz')
        labels = read_labels(MNIST_ROOT / 't10k-labels-idx1-ubyte.gz')
        images = images * (1. / 255.)
...
EMNIST_ROOT = pathlib.Path('./data/emnist')

class letters_prep:

    def __init__(self):
        images = read_images(
            EMNIST_ROOT / 'emnist-letters-train-images-idx3-ubyte.gz')
        labels = read_labels(
            EMNIST_ROOT / 'emnist-letters-train-labels-idx1-ubyte.gz')
        images = images * (1. / 255.)
        images = images.reshape((images.shape[0], 28, 28, 1))
        images = np.rot90(images, k=3, axes=(1, 2))
        images = np.flip(images, axis=2)

        self.X_train, self.X_val, self.Y_train, self.Y_val = train_test_split(images, labels, train_size=0.8,
                                                                              test_size=0.2)
        ...
        images = read_images(
            EMNIST_ROOT / 'emnist-letters-test-images-idx3-ubyte.gz')
        labels = read_labels(
            EMNIST_ROOT / 'emnist-letters-test-labels-idx1-ubyte.gz')
        images = images * (1. / 255.)
        images = images.reshape((images.shape[0], 28, 28, 1))
        images = np.rot90(images, k=3, axes=(1, 2))
        images = np.flip(images, axis=2)
